...
; ---------------------------------
    ...
    ; 0. 禁用中断（非常重要！）
    ;    如果不禁用，硬件中断会导致系统崩溃（因为还没有IDT）
    CLI

    ; 1. 写入启动信息到内存（struct BOOTINFO）
    MOV BYTE [0x0ff0], CYLS          ; cyls: 柱面数
    MOV BYTE [0x0ff1], 0             ; leds: 键盘LED状态（暂时为0）
    MOV BYTE [0x0ff2], 0x13          ; vmode: 显示模式（VGA 0x13）
    MOV BYTE [0x0ff3], 0             ; reserve: 保留
    MOV WORD [0x0ff4], 320           ; scrnx: 屏幕宽度
    MOV WORD [0x0ff6], 200           ; scrny: 屏幕高度
    MOV DWORD [0x0ff8], 0x000a0000   ; vram: 显存地址
